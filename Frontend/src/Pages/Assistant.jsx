...
export default function Assistant() {
  ...
  // Har naye message ke baad bottom par scroll karne ke liye
  useEffect(() => {
  ...
  const askBot = async () => {
    ...
    try {
      ...
      // Agar token purana ya expire ho gaya ho
      if (res.status === 401) {
      ...
      // Backend key "response" ko frontend state mein add karein
    ...
    <div className="flex flex-col h-[85vh] bg-gray-50 max-w-5xl mx-auto my-6 rounded-[2.5rem] shadow-2xl overflow-hidden border border-white">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-white/50 backdrop-blur-sm">
        {chat.length === 0 && (
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="italic">How can I assist you with your medical query today?</p>
          </div>
        )}
        ...
        {chat.map((m, i) => (
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`flex gap-3 max-w-[85%] ${m.role === "user" ? "flex-row-reverse" : ""}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${m.role === "user" ? "bg-green-700 text-white" : "bg-green-100 text-green-700"}`}>
                {m.role === "user" ? <FaUser size={12}/> : <FaRobot size={12}/>}
              </div>
              <div className={`p-4 rounded-2xl shadow-sm text-sm ${m.role === "user" ? "bg-green-700 text-white rounded-tr-none" : "bg-white text-gray-800 rounded-tl-none border border-green-50"}`}>
                {m.text}
              </div>
            </div>
          </div>
        ))}
        {loading && <div className="text-green-600 animate-pulse text-[10px] font-bold ml-12">MediBot is typing...</div>}
        <div ref={endRef} />
      </div>
      <div className="p-6 bg-white border-t flex gap-4">
        ...
        <button 
          onClick={askBot} 
          disabled={loading}
          className={`p-5 rounded-2xl shadow-lg transition-all ${loading ? 'bg-gray-300' : 'bg-green-700 text-white hover:bg-green-800 active:scale-95'}`}
        >
          <FaPaperPlane />
        </button>
      </div>
